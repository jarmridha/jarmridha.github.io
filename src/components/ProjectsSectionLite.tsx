...
function ProjectCard({project}: {project: typeof projects[number]}) {
 ...
 useEffect(() => {
  if (!playing || project.photos.length < 2) return;
  const timer = window.setInterval(() => {
   setActive(current => (current + 1) % project.photos.length);
   setFailed(false);
  }, 3000);
  return () => window.clearInterval(timer);
 }, [playing, project.photos.length]);
 ...
 return <article className="glass-card rounded-xl overflow-hidden">
  ...
  {project.photos.length>1 && <div className="mt-4 flex flex-wrap gap-2" aria-label={`${project.title} photos`}><button type="button" onClick={()=>setPlaying(value=>!value)} aria-label={`${playing ? "Pause" : "Play"} ${project.title} slideshow`} className="h-11 px-4 rounded-lg border border-primary text-primary">{playing ? "Pause" : "Play"}</button>{project.photos.map((name,i)=><button key={name} type="button" onClick={()=>select(i)} aria-label={`Show ${project.title} photo ${i+1}`} aria-pressed={active===i} className={`h-11 w-11 rounded-lg border ${active===i?'border-primary bg-primary text-primary-foreground':'border-border text-foreground'}`}>{i+1}</button>)}</div>}
  </div></article>;
}
