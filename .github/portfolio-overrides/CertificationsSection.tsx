...
const certs: Certificate[] = [
  ...
  { title: "ISO 45001:2018 Awareness", image: certIso45001, pdf: "/certificates/iso-45001-2018.pdf", issuer: "Issuer not stated in stored PDF", date: "25 Sep 2024" },
  { title: "ISO 14001:2015 Awareness", image: certIso14001, pdf: "/certificates/iso-14001-2015.pdf", issuer: "Issuer not stated in stored PDF", date: "23 Sep 2024" },
  ...
  return (
    <section id="certifications" className="section-padding bg-surface/50">
      <div className="max-w-7xl mx-auto">
        <SectionHeading title="Certifications & Trainings" subtitle="Verified credentials with source PDF, issuer and issue date" />
          ...
          {certs.map((c, i) => {
            ...
            return (
              <motion.button type="button" key={c.title} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: i * 0.04 }} className="glass-card-hover rounded-lg p-4 text-left flex items-start gap-3 cursor-pointer relative focus:outline-none focus:ring-2 focus:ring-primary/60" onMouseEnter={() => setPreviewIdx(i)} onMouseLeave={() => setPreviewIdx(null)} onFocus={() => setPreviewIdx(i)} onBlur={() => setPreviewIdx(null)} onClick={() => setModalCert(c)} aria-label={`Open certificate: ${c.title}`}>
                ...
                <span><span className="block text-sm font-body text-foreground">{c.title}</span><span className="block mt-1 text-xs text-muted-foreground font-body">{c.issuer}</span><span className="block mt-0.5 text-[11px] text-muted-foreground font-body">Issued: {c.date}</span></span>
                <AnimatePresence>
                  {previewIdx === i && (
