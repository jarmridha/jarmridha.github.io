...
type Certificate = { title: string; image: string; pdf: string; issuer?: string };

const certs: Certificate[] = [
  { title: "PMP Certification: Unit 3", image: certPmpUnit3, pdf: "/certificates/pmp-unit-3.pdf", issuer: "Coursera" },
  { title: "HSE Engineering", image: certHseEngineering, pdf: "/certificates/hse-engineering.pdf", issuer: "Khalifa University" },
  { title: "Construction Project Management", image: certConstructionPM, pdf: "/certificates/construction-project-management.pdf", issuer: "Columbia University" },
  ...
  return (
    <section id="certifications" className="section-padding bg-surface/50">
      <div className="max-w-7xl mx-auto">
        <SectionHeading title="Certifications & Trainings" subtitle="Verified certificate files currently available in the portfolio" />
          ...
          {certs.map((c, i) => {
            ...
            return (
              <motion.button type="button" key={c.title} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: i * 0.04 }} className="glass-card-hover rounded-lg p-4 text-left flex items-start gap-3 cursor-pointer relative focus:outline-none focus:ring-2 focus:ring-primary/60" onMouseEnter={() => setPreviewIdx(i)} onMouseLeave={() => setPreviewIdx(null)} onFocus={() => setPreviewIdx(i)} onBlur={() => setPreviewIdx(null)} onClick={() => setModalCert(c)} aria-label={`Open certificate: ${c.title}`}>
                ...
                <span><span className="block text-sm font-body text-foreground">{c.title}</span>{c.issuer && <span className="block mt-1 text-xs text-muted-foreground font-body">{c.issuer}</span>}</span>
                <AnimatePresence>
                  {previewIdx === i && (
                    <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ duration: 0.2 }} className={`hidden sm:block absolute bottom-full mb-3 z-50 pointer-events-none w-[280px] max-w-[calc(100vw-2rem)] ${previewAlign}`}>
                      <div className="glass-card rounded-xl overflow-hidden border border-primary/20 shadow-lg"><img src={c.image} alt="" className="w-full h-auto object-cover" loading="lazy" /><div className="px-3 py-2 text-xs text-muted-foreground text-center font-body">{c.title}</div></div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.button>
      ...
      <AnimatePresence>
        {modalCert && (
          <motion.div role="dialog" aria-modal="true" aria-label={modalCert.title} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-background/80 backdrop-blur-md" onClick={() => setModalCert(null)}>
            <motion.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.92, opacity: 0 }} className="glass-card rounded-2xl overflow-hidden border border-primary/20 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
              <div className="relative"><img src={modalCert.image} alt={`${modalCert.title} certificate preview`} className="w-full h-auto object-cover" /><button type="button" onClick={() => setModalCert(null)} aria-label="Close certificate preview" className="absolute top-3 right-3 w-9 h-9 rounded-full bg-background/80 border border-border flex items-center justify-center text-foreground hover:text-primary focus:outline-none focus:ring-2 focus:ring-primary/60"><X size={16} aria-hidden="true" /></button></div>
              <div className="px-5 py-4 space-y-3"><div><h3 className="text-sm font-body font-medium text-foreground">{modalCert.title}</h3>{modalCert.issuer && <p className="mt-1 text-xs text-muted-foreground font-body">Issuer: {modalCert.issuer}</p>}</div><a href={modalCert.pdf} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-body text-primary hover:underline focus:outline-none focus:ring-2 focus:ring-primary/50 rounded">View full certificate (PDF) <ExternalLink size={12} aria-hidden="true" /></a></div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
