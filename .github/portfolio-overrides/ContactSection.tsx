        ...
        <div className="grid md:grid-cols-2 gap-8">
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }} className="glass-card rounded-xl p-6 space-y-5">
            ...
            <div className="space-y-4 text-sm font-body">
              <div className="flex items-center gap-3 text-muted-foreground"><Mail size={16} className="text-primary shrink-0" /><a href={`mailto:${CONTACT_EMAIL}`} className="hover:text-foreground transition-colors">{CONTACT_EMAIL}</a></div>
              ...
              <div className="flex items-center gap-3 text-muted-foreground"><MapPin size={16} className="text-primary shrink-0" /><span>Sreepur, Gazipur, Dhaka, Bangladesh</span></div>
            </div>
          </motion.div>
          ...
          <motion.form initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.1 }} className="glass-card rounded-xl p-6 space-y-4" onSubmit={handleSubmit} aria-label="Contact form">
            ...
            <button type="submit" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-primary text-primary-foreground font-body text-sm font-medium hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-primary/60 transition"><Send size={14} /> Email Me Directly</button>
            <p className="text-[11px] text-muted-foreground font-body">Your message opens addressed directly to {CONTACT_EMAIL} in your email app.</p>
          </motion.form>
        </div>
      </div>
    </section>
